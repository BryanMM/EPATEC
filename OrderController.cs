...
namespace WebApplication1.Controllers
{
    public class OrderController : ApiController
    {
        ...
        {
            string ConnectionString = "SELECT TOP(1) PRName,  SUM(PRAmount) AS Total FROM " + baseString + "WHERE ";
            string LogicalConnector = "OR";
            if (ids == null)
            {
                return "SELECT TOP(1) PRName, SUM(PRAMount) AS Total FROM " + baseString + "GROUP BY PRName ORDER BY SUM(PRAmount) DESC;";
            }
            if (attr.Length > 1)
            {
        ...
        public JsonResult<List<Order>> GetMostSold(string attribute, string id)
        {
            string[] attr = attribute.Split(',');
            string[] ids = id.Split(',');
            List<Order> values = new List<Order>();
